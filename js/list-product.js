...
$(function () {
  ...
  nextTo(currentPage);
  const numberOfPage = Math.round(products.length / numberElOfPage);
  ...
  pageItemView.outerHTML = arr
    .map((item) => {
      ...
      return `<li class="page-item"><a class="page-link page-item-number" href="list-product.html?page=${pageNumber}">${pageNumber}
          </button></li>`;
    })
    .join("");
});
